...
public:
  /**
    ...
    */
  BatteryService(BLEDevice& _ble, ServiceDataHandler* serviceDataHandler, uint8_t level = 100, void (*callback)(GattCharacteristicReadAuthCBParams*) = NULL) :
    ble(_ble),
    _pServiceDataHandler(serviceDataHandler),
    batteryLevelCharacteristic(GattCharacteristic::UUID_BATTERY_LEVEL_CHAR, &level, sizeof(level), sizeof(level),
                               GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_READ | GattCharacteristic::BLE_GATT_CHAR_PROPERTIES_NOTIFY)
  {
    ...
    batteryLevelCharacteristic.setReadAuthorizationCallback(callback);
  ...
  {
    _pServiceDataHandler->push(batteryLevelCharacteristic.getValueAttribute().getHandle(), &newLevel, 1);
  }
